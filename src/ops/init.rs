...
/// entries and never entering nested repositories.
fn find_descendant_config(dir: &Path) -> Result<Option<PathBuf>> {
    for entry in std::fs::read_dir(dir).map_err(|e| Error::io("listing", dir, e))? {
        ...
        if let Some(found) = find_descendant_config(&path)? {
...
pub fn init(opts: &InitOpts) -> Result<()> {
    ...
    if let Some(found) = find_descendant_config(&cwd)? {
        return Err(Error::Usage(format!(
    ...
    report::out(format!("initialized `{}`", cwd.join(CONFIG_NAME).display()));
    report::note(
