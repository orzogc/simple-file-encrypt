...
- 确定性加密只保护**不可预测**的内容：随机 token 和密钥是安全的，可猜测的配置行无需密钥即可被验证猜测。
- 文本模式只有**单元级完整性**：拥有写权限的攻击者可以不被察觉地重排、删除、复制或复活真实的密文行——这是按行合并的代价，请靠审查 git 历史来兜底。

如果你需要隐藏文件结构与修改模式，请改用
[git-simple-encrypt](https://github.com/orzogc/git-simple-encrypt) 或
`age`/`git-crypt` 一类工具。完整分析见
...
- 每个单元（文本行、空文件标记、64 KiB 二进制分块）都用 **AES-CMAC-SIV（RFC 5297，AES-256）** 加密，密钥由 BLAKE3 从域密钥和文件的仓库相对路径派生。没有 nonce、加密时没有随机性：密文是 `(域密钥, 路径, 内容)` 的纯函数。
